...
//$lres = mssql_query($lsql,$conn);
    ...
    $lsql = "SELECT * FROM league WHERE ([session] = 'Summer') and regview = 1 ORDER BY [order]";
...
<label>
<div align="center">
  <p><span class="style1">List registered teams for</span>
    <select name="league" tabindex="2" OnChange="location.href=this.value">
     ...
     <  <option value="leagueinfo.php?l=<?php echo $lrow['l_id'];?>"><?php echo "$lrow[session] $lrow[night] $lrow[type] $lrow[size]'s $lrow[division]"; ?></option>
      <?php
}
